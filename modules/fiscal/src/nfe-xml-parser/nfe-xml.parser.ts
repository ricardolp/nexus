...
function xmlInvalid(): never {
  throw new ValidationError('nfe_xml_invalid');
}
...
function pickUnit(prod: Record<string, unknown>): string {
  const uCom = pickString(prod, "uCom")?.trim();
  if (uCom) return uCom;
  const uTrib = pickString(prod, "uTrib")?.trim();
  if (uTrib) return uTrib;
  return "UN";
}
...
function findTaxGroupBase(
  imposto: Record<string, unknown> | null,
  taxKey: "PIS" | "COFINS",
): number {
  if (!imposto) return 0;
  const tax = asRecord(imposto[taxKey]);
  if (!tax) return 0;

  for (const value of Object.values(tax)) {
    const group = asRecord(value);
    const vbc = pickDecimal(group, "vBC");
    if (vbc > 0) return vbc;
  }
...
/** Valor líquido do item para MIRO: base PIS/COFINS do XML ou vProd − vICMS. */
export function resolveItemMiroAmount(
  imposto: Record<string, unknown> | null,
  valorProd: number,
): number {
  const pisBase = findTaxGroupBase(imposto, "PIS");
  if (pisBase > 0) return roundMoney(pisBase);

  const cofinsBase = findTaxGroupBase(imposto, "COFINS");
  if (cofinsBase > 0) return roundMoney(cofinsBase);

  const icms = findItemIcms(imposto);
  return roundMoney(Math.max(0, valorProd - icms));
}
...
function parseItems(infNFe: Record<string, unknown>): ParsedNfeItem[] {
  ...
  return asArray(infNFe.det)
    .map((detRaw, index) => {
      ...
      const valorTotal = pickDecimal(prod, "vProd");
      ...
      return {
        ...
        unidade: pickUnit(prod),
        valorUnitario: pickDecimal(prod, "vUnCom"),
        valorTotal,
        itemAmount: resolveItemMiroAmount(imposto, valorTotal),
        icms: findItemIcms(imposto),
        ...(xPed ? { xPed } : {}),
        ...(nItemPed ? { nItemPed } : {}),
      };
    })
...
export function resolveOrganizationCompanyDocument(parsed: ParsedNfeXml): string {
  ...
  if (!parsed.recipientDocument) xmlInvalid();
