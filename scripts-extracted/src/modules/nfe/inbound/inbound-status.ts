import type { nfeInboundStatusEnum } from "../../../db/nfe-schema.js";

export type NfeInboundStatus = (typeof nfeInboundStatusEnum.enumValues)[number];
...
const ALLOWED_TRANSITIONS: Record<NfeInboundStatus, NfeInboundStatus[]> = {
  ...
  pedido_matched: ["delivery_creating", "rejected_inbound"],
...
export function canTransitionInbound(from: NfeInboundStatus, to: NfeInboundStatus): boolean {
  return ALLOWED_TRANSITIONS[from].includes(to);
}

export const TIMELINE_STEP_ORDER: { key: string; label: string; minStatus: NfeInboundStatus }[] = [
  { key: "xml", label: "Importada por XML", minStatus: "xml_imported" },
  { key: "sefaz", label: "Validação SEFAZ", minStatus: "sefaz_validated" },
  { key: "pedido", label: "Validação pedido (xPed / nItem)", minStatus: "pedido_matched" },
...
export function getStatusesForStatusInterno(statusInterno: StatusInterno): NfeInboundStatus[] {
  const all = Object.keys(INBOUND_STATUS_LABELS) as NfeInboundStatus[];
  return all.filter((s) => mapInboundToStatusInterno(s) === statusInterno);
}
